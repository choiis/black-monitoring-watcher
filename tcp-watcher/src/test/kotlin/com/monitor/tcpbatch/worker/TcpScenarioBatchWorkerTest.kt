...
class TcpScenarioBatchWorkerTest {
    ...
    private val instanceId = "test-tcp-instance-001"

    private lateinit var worker: TcpScenarioBatchWorker

    @BeforeEach
    fun setUp() {
        worker = TcpScenarioBatchWorker(curatorFramework, repository, instanceId)
    }
    ...
    @Test
    fun `getTcpScenarioList should return empty list initially`() {
        val scenarios = worker.getTcpScenarioList()
    ...
    fun `floorMod should handle negative hash values correctly`() {
        ...
        val key = TcpScenarioKey(UUID.randomUUID(), negativeHashUuid)
        ...
        // Should match exactly one partition (not negative index)
        for (index in 0 until total) {
            if (method.invoke(worker, key, index, total) as Boolean) {
                matchedCount++
